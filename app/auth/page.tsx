...
export default function AuthPage() {
  ...
  const handleAuth = async (e: React.FormEvent) => {
    ...
    // Dummy auth logic
    if (activeTab === 'signin' && email === 'admin' && password === 'admin') {
      setMessage('Login successful! Redirecting...')
      setTimeout(() => {
        router.push('/dashboard')
      }, 1000)
    } else if (activeTab === 'signup' && email && password && name) {
      setMessage('Account created successfully! Redirecting...')
      setTimeout(() => {
        router.push('/dashboard')
      }, 1000)
    } else if (activeTab === 'magic' && email) {
      setMessage('Magic link sent to your email!')
    } else if (activeTab === 'reset' && email) {
      setMessage('Password reset link sent to your email!')
    } else {
      setMessage('Invalid credentials or missing fields')
    }
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-blue-50 via-indigo-50 to-slate-100 flex items-center justify-center px-4 py-8">
        ...
        <div className="bg-white rounded-2xl shadow-xl border border-slate-200 overflow-hidden">
          <div className="px-8 pt-8 pb-6">
            <div className="text-center mb-8">
              ...
              <h1 className="text-2xl font-bold text-slate-800 mb-2">Welcome to StayEase</h1>
              <p className="text-slate-600">Your perfect stay awaits</p>
            </div>
            ...
            <form onSubmit={handleAuth} className="space-y-4">
              ...
              >
                {isLoading ? 'Processing...' : 
                  ...
                  activeTab === 'magic' ? 'Send Magic Link' : 'Reset Password'}
              </button>
            </form>
            ...
            <div className="mt-6">
              ...
              <div className="mt-4 grid grid-cols-2 gap-3">
                <button
                  ...
                  className="flex items-center justify-center px-4 py-3 border border-slate-300 rounded-lg hover:bg-slate-50 transition-all group"
                >
                ...
                <button
                  ...
                  className="flex items-center justify-center px-4 py-3 border border-slate-300 rounded-lg hover:bg-slate-50 transition-all"
                >
                  ...
                  <span className="text-slate-700 font-medium">Magic Link</span>
                </button>
              </div>
            </div>
          </div>
